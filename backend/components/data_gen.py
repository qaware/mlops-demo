...
def data_gen(words: dict, storage_type: str, train_data: OutputPath(Dataset),
    ...
    def prepare_data(words):
        ...
        predefined_good_sentences = [
            ...
            "Software engineering isa key driver in the development of green technology and sustainable solutions.",
        ...
        predefined_bad_sentences = [
            ...
            "The weather is chaotic, with sudden stormsand tornado's which destroy all life.",
            ...
            "Freezing rain contributes to the weather's misery.","The Weather is bad",
            ...
            "Lingering clouds make the weather gloomy for daysand years to come.",
    ...
    def store_data_mongodb(data_x, label_x):
        from pymongo import MongoClient
        mongodb_uri = f'mongodb://root:example@mongodb:27017'
        client = MongoClient(mongodb_uri)
        db = client['mlops-demo']

        data_collection = db['data']
        data_documents = [{'text': text, 'label': label} for text, label in zip(data_x, label_x)]
        data_collection.insert_many(data_documents)

        tokenizer_collection = db['tokenizer']
        padded_sequences_collection = db['padded_sequences']
        tokenizer_document = {'_id': 'tokenizer', 'object': pickle.dumps(tokenizer)}
        padded_sequences_document = {'_id': 'padded_sequences', 'object': pickle.dumps(padded_sequences)}

        tokenizer_collection.replace_one({'_id': 'tokenizer'}, tokenizer_document, upsert=True)
        padded_sequences_collection.replace_one({'_id': 'padded_sequences'}, padded_sequences_document, upsert=True)

        progress_collection = db['progress']
        progress_document = {'_id': 'data_gen', 'status': 'done'}
        progress_collection.replace_one({'_id': 'data_gen'}, progress_document, upsert=True)
    ...
    data_x, label_x = prepare_data(words)
    ...
    if storage_type == 'mongodb':
        store_data_mongodb(data_x, label_x)
